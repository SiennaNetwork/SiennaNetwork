...
## ETHEREUM
## Deploying WSIENNA (wrapped SIENNA) - ERC20
...
### Place the deploying seed in a `.secret` file in the root directory of the project.
...
Open `migrations/2_deploy_contracts.js` and make sure the address for the bridge is the correct one, depending on the network you are planning to deploy the contracts on.

Replace the address on line 6 with the address for the bridge for the relevant network you are planning to deploy on. Take a look at the table below:
...
Make sure you're running in a secure environment with a reliable Internet connection.
...
Make sure that you have the address and mnemonic ready for the mainnet wallet that you will use to deploy the contracts. Make sure the wallet has at least 8 SCRT in it, to cover gas costs for all the deployments on Secret Network.

## Deploying

⚠️ **Make sure you have the prepared initial vesting schedule in `settings/schedule.ods` before proceeding further.**
...
You should now have 3 contracts: **TOKEN**, **MGMT** and **RPT**, owned by your individual account.
They are all interconnected - MGMT and RPT point to each other, and MGMT owns TOKEN.
Hence, to transfer control over them to the multisig account. you need to do perform the following steps:

### Add deployment key to `secretcli`

If you haven't already, make sure you have the key used for deploying the contracts stored in `secretcli`.

```bash
# Add deployment key to secretcli
secretcli keys add DEPLOYMENT_KEY --recover
```

The command above will ask you to for the mnemonic seed phrase so it can import your account.

```bash
# To confirm the above command worked:
secretcli keys show -a DEPLOYMENT_KEY
```

You should now see the address corresponding to you deployment key, e.g.:
