...
describe("IDO", () => {
  ...
  it("Attempt instantiate and swap with a custom buying token", async function () {
    ...
    const sellingViewkey = (await context.sellingToken.createViewingKey(buyer)).key;
    ...
    const buyBalanceAfter = await context.buyingToken.balance(buyer.address, buyViewkey);
    const sellBalanceAfter = await context.sellingToken.balance(buyer.address, sellingViewkey);
