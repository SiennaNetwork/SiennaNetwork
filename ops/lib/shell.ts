...
export default async function shell (context = {}) {
  ...
  console.info('  '+['network', 'agent', 'builder'].join('\n  '))
  const loop = repl.start({
    prompt: `${context.network}> `,
    //writer: x => render(x)
  })
  await new Promise((resolve, reject)=>
    loop.setupHistory('.fadroma_repl_history', (err, repl) => err ? reject(err) : resolve(repl)))
  Object.assign(loop.context, { Contracts, Ensembles, network, agent, builder })
}
