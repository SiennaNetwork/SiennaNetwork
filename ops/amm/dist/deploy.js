...
async function deploy() {
    const client = await build_client(process.env.MNEMONIC, process.env.SECRET_REST_URL);
    ...
    const factory_instance = await client.instantiate(result.factory.id, factory_init_msg, `${commit} - SIENNA AMM FACTORY`, undefined, undefined, create_fee('200000'));
    writer.write(new ContractInfo(result.factory.code_hash, factory_instance.contractAddress), `addresses/factory`);
}
