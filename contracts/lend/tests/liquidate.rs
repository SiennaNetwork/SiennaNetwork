...
fn liquidate_gain_interest() {
    ...
    lend.prefund_user(ALICE, Uint128(20 * one_token(18)), underlying_2.clone());
    // deposit
    lend.ensemble
        .execute(
            &Snip20HandleMsg::Send {
                recipient: market_2.contract.address.clone(),
                recipient_code_hash: None,
                amount: Uint128(10 * one_token(18)),
                msg: Some(to_binary(&market::ReceiverCallbackMsg::Deposit {}).unwrap()),
                memo: None,
                padding: None,
            },
    ...
    // crash the price of first token and liquidate
    lend.set_oracle_price(market_1.symbol.as_bytes(), Uint128(8 * one_token(17)))
        .unwrap();
    ...
    // TODO: block height is ignored for some reason?
    // let mock_env = MockEnv::new(ALICE, underlying_2.clone()).height(12347);
    // lend.ensemble
    //     .execute(
    //         &Snip20HandleMsg::Send {
    //             recipient: market_2.contract.address.clone(),
    //             recipient_code_hash: None,
    //             amount: Uint128(1_500_000_000_000_000_000),
    //             msg: Some(
    //                 to_binary(&market::ReceiverCallbackMsg::Liquidate {
    //                     borrower: id,
    //                     collateral: market_1.contract.address.clone(),
