...
fn init() {
    ...
    let config: mgmt::ConfigResponse = tge.ensemble.query(
        tge.mgmt.address.clone(),
        mgmt::QueryMsg::Config { }
    ).unwrap();
    ...
    let schedule: Schedule<HumanAddr> = tge.ensemble.query(
        tge.mgmt.address,
        mgmt::QueryMsg::Schedule { }
    ).unwrap();
...
fn configure() {
    ...
    tge.ensemble.execute(
        &mgmt::HandleMsg::Configure { schedule: original.clone() },
    ...
    tge.ensemble.execute(
        &mgmt::HandleMsg::Configure { schedule: prod.clone() },
        tge.get_mgmt_env_as_admin()
    ).unwrap();
...
fn launch() {
    ...
    let err = tge.ensemble.execute(
        &mgmt::HandleMsg::Launch { },
        tge.get_mgmt_env("rando").time(launch_time)
    ).unwrap_err();
    ...
    tge.ensemble.execute(
        &mgmt::HandleMsg::Launch { },
        tge.get_mgmt_env_as_admin().time(launch_time)
    ).unwrap();
...
fn claim() {
    ...
    tge.ensemble.execute(
        &mgmt::HandleMsg::Claim { },
        tge.get_mgmt_env(claimant).time(launch_time + interval)
    ).unwrap();

    let balance = tge.query_balance(claimant);
    assert_eq!(balance, amount);

    let mut history: mgmt::HistoryResponse = tge.ensemble.query(
        tge.mgmt.address,
...
fn prod_schedule_simulation() {
    ...
    tge.ensemble.execute(
        &mgmt::HandleMsg::Configure { schedule: schedule.clone() },
        tge.get_mgmt_env_as_admin()
    ).unwrap();
    ...
    for pool in schedule.pools.iter() {
        for account in pool.accounts.iter() {
            ...
            */
            ...
            if account.start_at > 0 { //funds are not unlocked before `start_at`
                let progress: mgmt::ProgressResponse = tge.ensemble.query(
                    tge.mgmt.address.clone(),
            ...
            if account.cliff > Uint128::zero() { // cliff
    ...
    let history: mgmt::HistoryResponse = tge.ensemble.query(
        tge.mgmt.address,
        mgmt::QueryMsg::History {
            pagination: mgmt::Pagination {
                start: 0,
