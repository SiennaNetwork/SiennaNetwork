...
pub trait StorageClient<S: Storage> {
    fn storage (&self) -> S;
}
...
impl <'s, S: Storage> Pool<'s, S> {
    ...
    pub fn update_locked (&mut self, balance: Amount) -> StdResult<()> {
        ...
        self.save(POOL_TIMESTAMP, now)?;
        self.save(POOL_LOCKED,    balance)?;
...
impl <'s, S: Storage> User <'s, S> {
    ...
    fn save <T: Serialize> (
        &mut self, key: &[u8], val: T
    ) -> StdResult<()> {
        self.pool.save(&key, &to_vec(&val)?);
        Ok(())
    }
    fn save_ns <T: Serialize> (
        &mut self, ns: &[u8], key: &[u8], val: T
    ) -> StdResult<()> {
        self.pool.save_ns(ns, key, val)?;
        Ok(())
    }
    ...
    fn update (&mut self, user_locked: Amount, pool_locked: Amount) -> StdResult<&mut Self> {
        ...
        let lifetime = self.lifetime()?;
        self.save_ns(USER_LIFETIME,  address.as_slice(), lifetime)?;// Always increment lifetime
        self.save_ns(USER_TIMESTAMP, address.as_slice(), now)?;// Set user's time of last update to now
        self.save_ns(USER_LOCKED,    address.as_slice(), user_locked)?;// Update amount locked
    ...
    pub fn reset_liquidity_ratio (&self) -> StdResult<()> {
        ...
        self.save_ns(USER_PRESENT, address.as_slice(), existed)?;
        Ok(())
    }
