...
use fadroma::{
    to_binary, Api, CanonicalAddr, Composable, Env, HandleResponse, HumanAddr, Querier, StdError,
    StdResult, Storage,
};
use serde::{Deserialize, Serialize};

use crate::permit::{Permission, Permit, PermitHandler};
...
pub const CONTRACT: &[u8] = b"/auth/contract";
    ...
    Self: Sized,
{
    fn init(
        ...
        self.save_contract_address(contract_address)?;
    ...
    fn handle(&mut self, env: Env, msg: AuthHandle) -> StdResult<HandleResponse> {
        match msg {
            AuthHandle::NominateAdmin { address } => self.nominate_admin(env, address),
            AuthHandle::BecomeAdmin {} => self.become_admin(env),
            AuthHandle::CreateViewingKey { entropy, .. } => self.create_vk(env, entropy),
            AuthHandle::SetViewingKey { key, .. } => self.set_vk(env, key.into()),
        }
    }
    ...
    fn load_contract_address(&self) -> StdResult<HumanAddr> {
        let address = self.get::<CanonicalAddr>(CONTRACT)?.unwrap();
        self.humanize(address)
    }
    ...
    fn check_vk(&self, address: &HumanAddr, provided_vk: &ViewingKey) -> StdResult<HumanAddr> {
    ...
    fn authenticate<P: Permission>(
        ...
        match method {
            AuthMethod::Permit(permit) => {
                let address = Self::load_contract_address(self)?;
                permit.validate_with_permissions(self, &address, vec![permission])
            }
            AuthMethod::ViewingKey { address, key } => Self::check_vk(self, &address, &key.into()),
        }
    }
}
...
mod test {
    ...
    fn test_vk_handle() {
        ...
        let request = Handle::SetViewingKey {
            key: new_vk.clone(),
            padding: None,
        };
        Contract::handle(deps, mock_env(user.clone(), &[]).clone(), request).unwrap();
        assert_eq!(
            ViewingKey(new_vk.clone()),
            Auth::load_vk(deps, &user).unwrap().unwrap()
        );
    ...
    fn test_admin_handle() {
        ...
        let result = Contract::handle(
            ...
            Handle::Auth(AuthHandle::NominateAdmin {
        ...
        let msg = AuthHandle::NominateAdmin {
    ...
    fn test_auth_query() {
        ...
        match Contract::query(deps, Query::Auth(AuthQuery::Admin)).unwrap() {
            Response::Auth(AuthResponse::Admin { address }) => assert_eq!(address, admin),
            _ => unimplemented!(),
        };
    }
}
