...
mod wasm {
    fadroma_bind_js::bind_js!(fadroma, super);
}
    ...
    C: Contract<S, A, Q>,
{
    fn dispatch_handle(self, core: &mut C, env: Env) -> StdResult<HandleResponse> {
        match self {
            Handle::Auth(msg) => Auth::handle(core, env, msg),
...
impl<S, A, Q, C> QueryDispatch<S, A, Q, C, Response> for Query
where
    S: Storage,
    A: Api,
    Q: Querier,
    ...
    ($Core:ty) => {
        ...
        impl<S: Storage, A: Api, Q: Querier> crate::migration::Immigration<S, A, Q> for $Core {
            fn handle_receive_migration(
                ...
                Account::from_addr(self, &migrant, env.block.time)?.commit_deposit(self, staked)?;
