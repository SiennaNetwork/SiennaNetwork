...
mod harness; use harness::Harness;
...
kukumba! {
    ...
    when  "someone inits without providing an asset token address" {
        let result = harness.init_partial(0, &admin)?;
    }
    ...
    when  "a stranger tries to provide an asset token address" {
        let result = harness.tx_set_token(2, &badman, "bad_addr", "bad_hash")?
    }
    then  "an error is returned and nothing changes" {
        assert_error!(harness.q_status(3u64), "not configured");
    }
    when  "the admin provides an asset token address" {
        let result = harness.tx_set_token(4, &admin, "ok_addr", "ok_hash")?
    }
    then  "the instance is ready"
    and   "it goes live when someone locks funds" {
        assert_error!(harness.q_status(1u64), "missing data");
    ...
    then  "they get an error" {
        assert_error!(
            harness.tx_retrieve(9, &bob, 1000u128),
    ...
    then  "they get an error" {
        assert_error!(
            harness.tx_claim(1, &alice),
            "never provided liquidity"
        );
    }
    when  "a provider claims their rewards"
    then  "the instance sends them reward tokens" {
        let result = harness.tx_lock(2, &alice, 100)?;
        let result = harness.tx_claim(3, &alice)?;
    }
    ...
    then  "they receive an increment" {
        let result = harness.tx_claim(4, &alice)?;
    }
